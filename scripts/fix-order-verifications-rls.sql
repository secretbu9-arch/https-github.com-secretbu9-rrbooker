-- Fix RLS policies for order_verifications table
-- This script addresses the "new row violates row-level security policy" error

-- First, let's check if the order_verifications table exists and its current RLS status
DO $$
DECLARE
    rec RECORD;
BEGIN
    -- Check if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_verifications') THEN
        RAISE NOTICE 'order_verifications table exists';
        
        -- Check RLS status
        IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'order_verifications' AND rowsecurity = true) THEN
            RAISE NOTICE 'RLS is enabled on order_verifications table';
        ELSE
            RAISE NOTICE 'RLS is disabled on order_verifications table';
        END IF;
        
        -- List current policies
        RAISE NOTICE 'Current policies on order_verifications:';
        FOR rec IN 
            SELECT policyname, permissive, roles, cmd, qual, with_check
            FROM pg_policies 
            WHERE tablename = 'order_verifications'
        LOOP
            RAISE NOTICE 'Policy: % | Permissive: % | Roles: % | Command: %', 
                rec.policyname, rec.permissive, rec.roles, rec.cmd;
        END LOOP;
        
    ELSE
        RAISE NOTICE 'order_verifications table does not exist - creating it';
        ...
        CREATE INDEX idx_order_verifications_status ON public.order_verifications(verification_status);
        CREATE INDEX idx_order_verifications_type ON public.order_verifications(verification_type);
        
        RAISE NOTICE 'order_verifications table created successfully';
    END IF;
END
$$;

-- Enable RLS on order_verifications table
...
-- Create comprehensive RLS policies for order_verifications
...
-- Policy 2: Allow system/service to create verifications (for authenticated users)
...
-- Policy 5: Allow barbers to view verifications for orders they're involved with
...
-- Create a function to handle verification creation with proper permissions
...
DO $$
BEGIN
    RAISE NOTICE '✅ RLS policies for order_verifications have been set up successfully';
    RAISE NOTICE '📋 Policies created:';
    RAISE NOTICE '  - Allow users to view their own verifications';
    RAISE NOTICE '  - Allow system to create verifications';
    RAISE NOTICE '  - Allow system to update verifications';
    RAISE NOTICE '  - Allow managers to view all verifications';
    RAISE NOTICE '  - Allow barbers to view relevant verifications';
    RAISE NOTICE '🔧 Helper functions created:';
    RAISE NOTICE '  - create_order_verification()';
    RAISE NOTICE '  - update_verification_status()';
END
$$;
