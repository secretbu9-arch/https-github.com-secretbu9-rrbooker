...
-- Create function to validate no overlapping day-offs
CREATE OR REPLACE FUNCTION validate_no_overlapping_day_offs()
RETURNS TRIGGER AS $$
BEGIN
  -- Check for overlapping day-offs
  IF check_day_off_overlap(NEW.barber_id, NEW.start_date, NEW.end_date, NEW.id) THEN
    RAISE EXCEPTION 'Overlapping day-off already exists for this barber in the specified date range';
  END IF;
...
-- Create trigger to prevent overlapping day-offs
DROP TRIGGER IF EXISTS prevent_overlapping_day_offs ON barber_day_offs;
CREATE TRIGGER prevent_overlapping_day_offs
  BEFORE INSERT OR UPDATE ON barber_day_offs
  FOR EACH ROW
  WHEN (NEW.is_active = true)
  EXECUTE FUNCTION validate_no_overlapping_day_offs();
